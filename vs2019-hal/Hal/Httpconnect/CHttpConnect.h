#pragma once
#ifndef _CHTTPCONNECT_H
#define _CHTTPCONNECT_H
#include <afxinet.h>
...
public:
	...
	int HttpGet(LPCTSTR strUrl, CString& strResponse, CString& strStatusCode, CString strHeader = _T(""));
	int HttpPost(LPCTSTR strUrl, CString& strResponse, LPCTSTR strPostData, CString& strStatusCode, CString strHeader = _T(""));

	CString Base64Encode(CString src, int srclen);
private:
	int ExecuteRequest(LPCTSTR strMethod, LPCTSTR strUrl, LPCTSTR strPostData, CString& strResponse, CString& strStatusCode, CString strHeader);
	CStringA GetReqData(const CString& szReqData);
	CString  GetResData(const CStringA& szResData);
	void Clear();
private:
	CInternetSession* m_pSession;						//定义一个Internet会话指针
	CHttpConnection* m_pConnection;						//Http连接指针
	CHttpFile* m_pFile;									//Http文件指针
	CString m_szReqCode;								//Http请求信息字符编码
